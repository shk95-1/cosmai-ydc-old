...
# **Europe PMC · 92개월 전체 기준이다.** 현준님 EXP-007 은 PubMed 2025 로 재서
# 값이 더 낮다(아데노신 2.7% · 엑소좀 7.6%). 순서는 양쪽에서 같아 결론은 안 바뀐다
RETENTION = [
    ("아데노신", 19.0, "현준님이 막은 것"),
    ("엑소좀", 20.6, "우리가 쓰려던 것"),
    ("트라넥삼산", 30.7, ""),
...
/* ── 4. 검색 계층 ─────────────────────────── */
sec('04','검색 — 만들고 측정했다',
 '어휘 검색(BM25)과 의미 검색(벡터)을 같은 질의 61개로 채점했습니다. <b>채택 기준은 결과를 보기 전에 정했습니다</b> — heldout 에서 BM25 는 구조적으로 0이므로 "0을 넘느냐"가 기준이었습니다. 아래 값은 모두 <b>e5all 2차</b> 벡터입니다.',
 ...
 +panel({src:'reports/retrieval_eval_{mode}_{engine}_{vectors}.csv', w:'h',
   title:'벡터가 깎인 원인은 접두어가 아니었다', note:'세 상태를 갈라 재면 하락이 어디서 났는지 보입니다.',
   body:gbar([{l:'heldout\\nP@10',v:[0.042,0.038,0.038]},
              {l:'heldout\\nHit@10',v:[0.17,0.15,0.13]},
              {l:'literal\\nP@10',v:[0.567,0.538,0.541]}],
     ['e5base','e5all 1차','e5all 2차'],{h:210,max:0.62,td:3,b:52,aria:'세 상태 재측정'}),
   legend:leg(['e5base — 식약처 벡터 0건','e5all 1차 — mfds 1차 청크','e5all 2차 — 보고번호 접두']),
   cap:'<b>접두어는 P@10 을 한 자리도 깎지 않았습니다.</b> 하락은 <b>식약처·성분 문서를 색인에 넣은 순간</b>(1차) 이미 났고, 접두어의 대가는 heldout 질의 <b>한 개</b>와 mfds 가 차지한 칸 23→29 입니다. 초판은 이 칸 수로 P@10 하락을 설명했는데 <b>칸 수는 1↔2차 양인데 P@10 은 e5base↔2차 양</b>이었습니다 — 한 문장에 기준선이 둘이었습니다.'})
 +panel({src:'측정값 요약',
