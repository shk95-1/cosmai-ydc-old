...
| 논문 | `data/external/paper_trend.csv` 3,128행 · `paper_growth.csv` 34행. **17검색어** × 2소스 × 92개월. 출처 `slopindustries/cosmai-ml` |
...
| **엑소좀 — 세 축이 같은 방향** | 논문 1.17·1.33 (둘 다 1 초과 · `usable`) · 검색 52배 · **선케어 처방 0.2%(1제품) · 배합순위 136위** |
| **레티날 — 논문이 반증한다** | 검색 287배인데 논문 **0.53·0.46 으로 뒷걸음.** 카드로 쓰지 않는다. `retinaldehyde` 로 봐야 한다 (`retinol` 아님) |
| **PDRN·트라넥삼산** | 검색 45·39배인데 **선케어 처방 0제품** — 카테고리 밖 성분 |
| **논문 색인 보정** | `cosmetic` 으로 나누면 EPMC·PubMed 차이가 평균 **0.083** 로 좁혀짐 |
| 논문 배수 못 쓰는 것 | PDRN · 판테놀 · 마데카소사이드 · 에틸헥실트리아존 (`ratio_usable=False`) |
| 나이아신아마이드 논문 | **두 색인이 갈린다** (1.00 vs 0.63 · 차이 0.37). 처방 근거는 쓰되 **논문 추세는 인용하지 않는다** |
