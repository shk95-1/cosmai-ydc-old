...
| 레티날 — 카드로 쓰지 않는다 | 검색 287배 · 처방 1.2% · 배합순위 93위. **논문 근거는 철회했다.** 광민감성 때문에 선케어에 넣기 어려운 성분이라는 게 이유고, 그건 우리가 측정한 값이 아니라 도메인 지식이다. 자동 카드로 안 내고 문서에 글로 남긴다 |
| **PDRN·트라넥삼산** | 검색 45·39배인데 **선케어 처방 0제품** — 카테고리 밖 성분 |
| ~~논문 색인 보정~~ | **철회.** `cosmetic` 잔존율이 100.1% 라 **분모는 화장품 색인, 분자는 전분야 색인**이다. 모집단이 다른 값으로 나눴다. 차이 0.083 도 같은 상수로 나눠서 생긴 수렴이지 일치의 증거가 아니다 |
...
| 논문 잔존율 (측정) | `(skin)` 을 붙이면 남는 비율. **엑소좀 20.6% · 트라넥삼산 30.7% · 콜라겐 33.3% · 나이아신아마이드 33.4% · 히알루론산 42.9% · 시카 47.8%.** 아데노신 19.0% 와 같은 자리다. 기준선 `cosmetic` 만 100.1% |
...
**08.25 재측정 — 수호님 2차 청크 반영 후.** mfds 텍스트가 바뀌어 벡터를 다시 합쳤다.
