...
| NAVER 검색 트렌드 | **3002 포트** PostgREST. `raw_item.payload` 가 bytea hex. 128개월 |
| 논문 | `data/external/paper_trend.csv` 2,760행 · `paper_growth.csv` 30행. 92개월 |
| 벡터 합본 | `.cache/vectors/e5all` 306,178 (우리 297,392 + 수호님 8,786) |
...
| 08.25 | **정규화 시점** (수호님은 정규화 전 본문으로 인코딩) | 프로브 코사인이 0.9986 |
...
| **NAVER 성분 검색** | 레티날 287배 · 엑소좀 52배 · PDRN 45배 · 트라넥삼산 39배 (기준_세럼 대비) |
| **PDRN·트라넥삼산** | 검색 45·39배인데 **선케어 처방 0제품** — 카테고리 밖 성분 |
| **논문 색인 보정** | `cosmetic` 으로 나누면 EPMC·PubMed 차이가 평균 **0.083** 로 좁혀짐 |
| 논문 원본 배수 | 색인 성장률이 달라 그대로 쓰면 안 됨 (EPMC 2.10x · PubMed 1.44x) |
| **히알루론산·판테놀·시카** | 채택 60·37·41% 인데 **고함량 0~5%** — 널리 쓰지만 얕게 |
...
- ~~`academic` 스키마 노출~~ — **전제가 틀렸다.** 그 스키마는 석현님 DB 가 아니라
  현준님 DGX 사본에 있다. 대신 CSV 로 받았다 (`data/external/paper_*.csv`,
  출처 `slopindustries/cosmai-ml`). PostgREST 로 꼭 봐야 하면 석현님 DB 에
  적재하는 쪽이 맞다 — 소스를 하나로 유지해야 한다
- **아데노신 논문 수를 근거로 쓰지 말 것** (현준님 경고). 화장품 성분이기 전에
  생화학 기초 물질이라 월 900~2,600편의 대부분이 화장품과 무관하다.
  `adenosine AND (skin OR cosmetic OR dermatology)` 로 다시 받으면 쓸 수 있다
- **NAVER 선케어 요청에 기준선 없음** — `선크림` 같은 기준 그룹을 넣어야 추세를
  읽을 수 있다. 지금은 그룹 간 상대 비교만 가능하다 (현준님)
- 식약처 청크 본문에 등록번호가 없어 서로 다른 품목이 같은 텍스트가 된다 (수호님)
